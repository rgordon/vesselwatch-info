/*
	WSFValidDateRangeResponse.h
	The implementation of properties and methods for the WSFValidDateRangeResponse object.
	Generated by SudzC.com
*/
#import "WSFValidDateRangeResponse.h"

@implementation WSFValidDateRangeResponse
	@synthesize DateFrom = _DateFrom;
	@synthesize DateThru = _DateThru;

	- (id) init
	{
		if(self = [super init])
		{
			self.DateFrom = nil;
			self.DateThru = nil;

		}
		return self;
	}

	+ (WSFValidDateRangeResponse*) createWithNode: (CXMLNode*) node
	{
		if(node == nil) { return nil; }
		return (WSFValidDateRangeResponse*)[[WSFValidDateRangeResponse alloc] initWithNode: node];
	}

	- (id) initWithNode: (CXMLNode*) node {
		if(self = [super initWithNode: node])
		{
			self.DateFrom = [Soap dateFromString: [Soap getNodeValue: node withName: @"DateFrom"]];
			self.DateThru = [Soap dateFromString: [Soap getNodeValue: node withName: @"DateThru"]];
		}
		return self;
	}

	- (NSMutableString*) serialize
	{
		return [self serialize: @"ValidDateRangeResponse"];
	}
  
	- (NSMutableString*) serialize: (NSString*) nodeName
	{
		NSMutableString* s = [NSMutableString string];
		[s appendFormat: @"<%@", nodeName];
		[s appendString: [self serializeAttributes]];
		[s appendString: @">"];
		[s appendString: [self serializeElements]];
		[s appendFormat: @"</%@>", nodeName];
		return s;
	}
	
	- (NSMutableString*) serializeElements
	{
		NSMutableString* s = [super serializeElements];
		if (self.DateFrom != nil) [s appendFormat: @"<DateFrom>%@</DateFrom>", [Soap getDateString: self.DateFrom]];
		if (self.DateThru != nil) [s appendFormat: @"<DateThru>%@</DateThru>", [Soap getDateString: self.DateThru]];

		return s;
	}
	
	- (NSMutableString*) serializeAttributes
	{
		NSMutableString* s = [super serializeAttributes];

		return s;
	}
	
	-(BOOL)isEqual:(id)object{
		if(object != nil && [object isKindOfClass:[WSFValidDateRangeResponse class]]) {
			return [[self serialize] isEqualToString:[object serialize]];
		}
		return NO;
	}
	
	-(NSUInteger)hash{
		return [Soap generateHash:self];

	}

@end
